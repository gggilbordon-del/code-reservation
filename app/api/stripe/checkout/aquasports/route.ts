...
const BONO_PRICE_EUR = 120;
const SERVICE_NAME = "Safari en Moto de Agua (60 min).";
const COMPANY_PREFIX = "AQUA";
const COMPANY_PHONE = "+34 928 566 994 / +34 638 783 134 / +34 640 579 609";
...
export async function POST(req: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/actividades/aquasports/exito?session_id={CHECKOUT_SESSION_ID}&tipo=${safeType}&cantidad=${quantity}&nombre=${safeName}&email=${safeEmail}&para=${safeRecipient}&mensaje=${safeMessage}&prefijo=${COMPANY_PREFIX}&servicio=${safeService}&phone=${safePhone}`,
      cancel_url: `${baseUrl}/actividades/aquasports?status=cancel`,
      metadata: {
        bookingType: "bono_aquasports",
        voucherType,
