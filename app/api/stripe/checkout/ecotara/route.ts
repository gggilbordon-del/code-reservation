...
const BONO_PRICE_EUR = 85;
const SERVICE_NAME = "Bono Regalo 'Dia de Retiro & Yoga'.";
const COMPANY_PREFIX = "TARA";
const COMPANY_PHONE = "696 604 182";
...
export async function POST(req: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/actividades/ecotara/exito?session_id={CHECKOUT_SESSION_ID}&tipo=${safeType}&cantidad=${quantity}&nombre=${safeName}&email=${safeEmail}&para=${safeRecipient}&mensaje=${safeMessage}&prefijo=${COMPANY_PREFIX}&servicio=${safeService}&phone=${safePhone}`,
      cancel_url: `${baseUrl}/actividades/ecotara?status=cancel`,
      metadata: {
        bookingType: "bono_ecotara",
        voucherType,
