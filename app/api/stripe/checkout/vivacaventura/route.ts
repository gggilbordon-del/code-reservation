...
const BONO_PRICE_EUR = 59.0;
const SERVICE_NAME = "Bono Multiaventura";
const COMPANY_PREFIX = "VIVAC";
const COMPANY_PHONE = "673 970 527 (Comunidad educativa) · 683 165 966 (Particulares y Colectivos)";
...
export async function POST(req: Request) {
  try {
    ...
    const successUrl = new URL("/actividades/vivacaventura/exito", req.url);
    successUrl.searchParams.set("session_id", "{CHECKOUT_SESSION_ID}");

    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      success_url: successUrl.toString(),
      cancel_url: new URL("/actividades/vivacaventura", req.url).toString(),
    ...
    console.error("Error creando checkout /vivacaventura:", error);
