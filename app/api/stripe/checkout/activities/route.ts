...
const BONO_PRICE_EUR = 129;
const SERVICE_NAME = "Paseo Privado en Barco";
...
export async function POST(req: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url: `${baseUrl}/actividades/exito?session_id={CHECKOUT_SESSION_ID}&tipo=${safeType}&cantidad=${quantity}&nombre=${safeName}&email=${safeEmail}&para=${safeRecipient}&mensaje=${safeMessage}`,
      cancel_url: `${baseUrl}/actividades?status=cancel`,
      metadata: {
        bookingType: "bono_digital",
        voucherType,
