...
export default function RetirosExitoPage() {
  ...
  const brand = BRAND_BY_PREFIX[prefijo] || BRAND_BY_PREFIX.ZEN;

  return (
    <main className="min-h-screen bg-gradient-to-b from-amber-100 via-stone-50 to-white p-4">
      <section
        ref={ticketRef}
        className="mx-auto w-full max-w-3xl rounded-3xl border border-stone-200 bg-white/95 p-6 shadow-xl"
      ...
      >
        ...
        <h1 className="mt-2 text-2xl font-bold text-stone-900">Tu bono de retiro esta listo</h1>
        <p className="mt-1 text-sm text-slate-600">Ticket listo para guardar como PDF y canjear con codigo o QR.</p>
        ...
        <div className="mt-5 rounded-2xl border p-5" style={{ borderColor: brand.border, backgroundColor: brand.soft }}>
          ...
          <p className="mt-4 text-xs text-slate-600">Bono valido por 12 meses. Canje segun disponibilidad del alojamiento.</p>
        </div>
        ...
        <div className="mt-4 rounded-2xl border bg-white p-5" style={{ borderColor: brand.border }}>
          ...
          <div className="mt-4 flex flex-col items-center justify-center gap-3">
            {qrUrl ? (
              <img src={qrUrl} alt={`QR del bono ${code}`} className="h-44 w-44 rounded-xl border bg-white p-2" style={{ borderColor: brand.border }} />
            ) : (
              <div className="h-44 w-44 animate-pulse rounded-xl border" style={{ borderColor: brand.border, backgroundColor: brand.soft }} />
            )}
            {/* <p className="text-xs text-slate-500">Se esta generando automaticamente un PDF limpio del ticket.</p> */}
          </div>
        </div>
      </section>
      <div className="mx-auto mt-4 w-full max-w-3xl">
        <button
          ...
          className="w-full rounded-xl border border-stone-300 bg-white px-4 py-2 text-sm font-semibold text-stone-800 hover:bg-stone-50"
