...
const CONFIG_EMPRESA: EmpresaConfig = {
  nombreComercial: "AquaSports Gran Canaria",
  subtitulo: "Experiencias nauticas lideres en el sur de la isla.",
  ...
  precioBono: 120,
  validezMeses: 12,
  telefonoReserva: "+34 928 566 994 / +34 638 783 134 / +34 640 579 609",
};

export function BookingAquaSportsVoucher() {
  ...
  const applyDemoAutofill = async () => {
    ...
    setBuyerEmail("demo+aquasports@grancanaria.test");
    setBuyerPhone("600123456");
    setQuantity(1);

    if (voucherType === "regalo") {
      setRecipientName("Demo Recipient");
      setGiftMessage("Prepárate para una experiencia brutal sobre las olas en Gran Canaria.");
    }
  ...
  const payNow = async () => {
    ...
    try {
      ...
      if (isTestMode) {
        ...
        window.location.href = `/actividades/aquasports/exito?${qs.toString()}`;
        return;
      }

      const res = await fetch("/api/stripe/checkout/aquasports", {
        method: "POST",
      ...
      <div className="mx-auto w-full space-y-4">
        <div className="relative h-52 w-full overflow-hidden rounded-3xl border border-[#0070f3]/40 shadow-2xl sm:h-64">
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 text-white">
            <p className="text-xs uppercase tracking-[0.16em] text-blue-100">{CONFIG_EMPRESA.etiqueta}</p>
            <h1 className="mt-1 text-2xl font-semibold text-white sm:text-3xl">{CONFIG_EMPRESA.nombreComercial}</h1>
            <p className="mt-1 max-w-2xl text-sm text-blue-100 sm:text-base">{CONFIG_EMPRESA.subtitulo}</p>
          </div>
        </div>

        <Card className="w-full border-[#0070f3]/30 bg-gradient-to-b from-blue-50 via-white to-white shadow-2xl">
          <CardContent className="space-y-4">
            <div className="pt-5">
              ...
              />
              <CardTitle className="text-xl text-[#0049a8]">{CONFIG_EMPRESA.nombreComercial}</CardTitle>
              <CardDescription className="mt-1 text-blue-700">{CONFIG_EMPRESA.subtitulo}</CardDescription>
            </div>

            <div className="rounded-xl border border-[#0070f3]/30 bg-gradient-to-r from-[#0070f3] to-[#0058c2] p-4 text-white">
              <p className="mt-1 text-lg font-semibold">{CONFIG_EMPRESA.claim}</p>
              <p className="text-sm text-blue-100">{CONFIG_EMPRESA.descripcion}</p>
            </div>

            <div className="rounded-xl border border-[#0070f3]/25 bg-white p-4">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <button
                ...
                className={`rounded-xl border p-3 text-left transition ${
                  voucherType === "regalo" ? "border-[#0070f3] bg-blue-50 shadow-sm" : "border-blue-200 bg-white hover:bg-blue-50/40"
                }`}
              >
                <div className="flex items-center gap-2 text-[#0058c2]">
                  <Gift className="h-4 w-4" />
                  <p className="font-semibold">Bono regalo</p>
                </div>
                <p className="mt-1 text-sm text-slate-600">Regala adrenalina sobre el mar en Gran Canaria.</p>
              </button>
              ...
              <button
                ...
                className={`rounded-xl border p-3 text-left transition ${
                  voucherType === "ticket_abierto" ? "border-[#0070f3] bg-blue-50 shadow-sm" : "border-blue-200 bg-white hover:bg-blue-50/40"
                }`}
              >
                <div className="flex items-center gap-2 text-[#0058c2]">
            ...
            <div>
              <Label htmlFor="quantity-aquasports">Cantidad de bonos</Label>
              <Input
                id="quantity-aquasports"
                type="number"
                min={1}
                max={20}
                value={quantity}
            ...
            <div className="rounded-xl border border-blue-100 bg-blue-50/70 p-4">
              ...
              <div className="mt-3 flex items-center justify-between">
                ...
                <span className="text-2xl font-bold text-[#0049a8]">{total.toFixed(2)} EUR</span>
              </div>
            </div>
            ...
            {isTestMode ? (
              <button
                ...
                className="w-full rounded-lg border border-blue-300 bg-white px-3 py-2 text-sm font-medium text-[#0049a8] hover:bg-blue-50"
              >
                Probar con tarjeta de prueba
              </button>
            ) : null}

            <Button className="w-full bg-[#0070f3] hover:bg-[#005ed1]" onClick={payNow} disabled={loading || total <= 0}>
              {loading ? "Redirigiendo..." : isTestMode ? "Simular compra de bono" : "Pagar ahora con Stripe"}
            </Button>
            {isTestMode ? (
              <p className="text-center text-xs font-medium text-[#0049a8]">Estas en modo DEMO. No se realizara ningun cargo real en tu tarjeta.</p>
            ) : null}
          </CardContent>
        </Card>
      </div>
